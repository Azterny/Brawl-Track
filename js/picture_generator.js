    ...
    <div id="export-modal" class="export-modal-overlay">
        <div class="export-modal-content">
            ...
            <div id="modal-step-config">
                <h2 style="font-family: 'Lilita One', cursive; color: #ffce00; margin-top: 0;">Générer une Carte</h2>
                <p style="color: #ccc; font-size: 0.95rem;">Choisissez le format de l'image :</p>
                ...
                <div class="export-format-options">
                    <label class="export-format-card selected" id="card-vertical" onclick="selectExportFormat('vertical')">
                        ...
                        <div style="text-align: left;">
                            ...
                            <p>Parfait pour les stories et téléphones.</p>
                        </div>
                    </label>
                    <label class="export-format-card" id="card-horizontal" onclick="selectExportFormat('horizontal')">
                        ...
                        <div style="font-size: 2rem;">💻</div>
                        <div style="text-align: left;">
                            <h3>Paysage (PC/Twitter)</h3>
                            <p>Idéal pour les écrans larges et bannières.</p>
                        </div>
                    </label>
                </div>
                
                <button class="btn-3d btn-yellow w-auto" onclick="startImageGeneration()">
                    <img src="/assets/icons/picture.png" style="height: 18px; vertical-align: middle; margin-right: 8px;"> 
                    Générer l'image
                </button>
            </div>

            <div id="modal-step-loading" style="display: none;">
                <h2 style="font-family: 'Lilita One', cursive; margin-top: 0;">Création en cours...</h2>
                <img src="/assets/loading_icon.png" class="spin" style="width: 50px; margin: 20px auto; display: block;">
                <p id="export-status" style="color: #aaa; font-style: italic;">Préparation...</p>
            </div>

            <div id="modal-step-result" style="display: none;">
                <h2 style="font-family: 'Lilita One', cursive; color: #28a745; margin-top: 0;">Carte prête !</h2>
...
/* =========================================
   ...
   (C'est ici que tu ajouteras getClubTemplate plus tard)
   ========================================= */
...
async function startImageGeneration() {
    ...
    try {
        statusDiv.innerText = "Construction du template...";
        ...
        // ROUTAGE SELON LE TYPE DEMANDÉ
        if (currentExportType === 'player') {
            // ÉNORME OPTIMISATION : On utilise playerData global de dashboard.js !
            if (typeof playerData !== 'undefined' && playerData) {
                targetData = playerData;
            } else {
                throw new Error("Données du joueur introuvables.");
            }
            finalHtml = await buildPlayerTemplate(targetData, mode);
            fileName = `BrawlTrack_${targetData.name}_${mode}.png`;
        } 
        // else if (currentExportType === 'club') { finalHtml = await buildClubTemplate(...); }

        exportContent.innerHTML = finalHtml;
        statusDiv.innerText = "Rendu HD de l'image...";
        ...
        setTimeout(() => {
            html2canvas(exportContent, {
                ...
                previewArea.innerHTML = `
                    ...
                    <button class="btn-3d btn-green w-auto" onclick="downloadImage('${imgDataUrl}', '${fileName}')">
                ...
                console.error(err);
                statusDiv.innerHTML = `<span style='color: #ff5555;'>Erreur de rendu (Canvas).</span>`;
            });
        }, 800);
